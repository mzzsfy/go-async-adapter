        ...
        go func() {
            ...
            if noSend {
                for i := 0; i < sendEventNum; i++ {
                    ...
                    //fmt.Printf("%v,code %v,data:%v \n", c, code, string(data))
                    if err != nil {
                        if !strings.Contains(err.Error(), "use of closed network connection") {
                            t.Error(c, "read error: ", err, data)
                        }
                        return
                    } else if code.IsControl() {
                        println("control", code, data)
                        return
                    }
                }
            } else {
...
func BenchmarkSend(b *testing.B) {
    client.NewWsc = func(ws websocket.AsyncWebsocket) client.Wsc { return &client.WscDoNothing{Ws: ws} }
    port := 30000 + rand.Intn(9999)
    clientNum := 2
    startWait := 0
    sendWait := 0
...
func BenchmarkRead(b *testing.B) {
    ...
    client.NewWsc = func(ws websocket.AsyncWebsocket) client.Wsc { return &client.WscDoSend{Count: nowCount, Ws: ws} }
    port := 30000 + rand.Intn(9999)
    clientNum := 2
    startWait := 0
    sendWait := 0
